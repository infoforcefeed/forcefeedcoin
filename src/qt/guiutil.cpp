...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("forcefeedcoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("forcefeedcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "forcefeedcoin:");
    }
...
{
    QString ret = QString("forcefeedcoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "forcefeedcoin.desktop";
    return GetAutostartDir() / strprintf("forcefeedcoin-%s.lnk", chain);
}
